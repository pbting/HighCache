 ...
 *
 */
public abstract class LRFUAbstractCache extends AbstractMapCache {
	...
	//每个10分钟
	private final static Long delay = (long) (10*60*1000);
	
	//只开启一个任务即可，因此需要一次判断
	private static boolean flag = true ;
	
	public LRFUAbstractCache(){
		super();
	...
	private void initParameter(int capacity) {
		...
		Rc_ES = new ConcurrentHashMap<Float, Map>(capacity);
	}
	
	public LRFUAbstractCache(int capacity){
		super(capacity);
...
System.out.println("..............item put key :"+key);
		if(K_Rc.containsKey(key)){//已经包含该key，则该怎么办
			...
			if(ES == null){
				//表示系统的第一次
				ES = new Hashtable<Object, LRFUAbstractCache.SRUKey>();
			}
	...
	public void itemRetrieved(Object key) {
		...
		if(ES_next == null){
			ES_next = new ConcurrentHashMap<Object, LRFUAbstractCache.SRUKey>();
		}
...
System.out.println(removeLfuKey.key+"--------------->remove key is:"+removeKey+"-------->,cache rate:"+removeCacheRate);
	...
	public class SRUKey{
		...
		public SRUKey(Object key){
			...
			if(LRFUAbstractCache.this.createTime == null)
				LRFUAbstractCache.createTime = System.currentTimeMillis();
		...
		public Float getCacheRate(){
			//根据方差值来计算缓存响应比
			float factor = LRFUAbstractCache.this.getFactor(this);
			this.currentCacheRate =  Math.abs(Float.valueOf(this.count)/factor);
System.out.println("[HighCache:getCacheRate]-----<"+this.count+">------factor:"+factor+"-------"+this.key+"--------rc:"+this.currentCacheRate);
			return this.currentCacheRate;
		}
	}
	
	public abstract float getFactor(LRFUAbstractCache.SRUKey sruKey);
}
